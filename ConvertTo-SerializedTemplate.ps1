...
.PARAMETER Template
Specifies the template object or objects to be converted. Each template object should contain properties for both PSPKI and ADO templates.
...
"@)
    ...
    foreach ($temp in $Template) {
        ...
        $KU = if ([int]$temp.templatePSPKI.Settings.Cryptography.CNGKeyUsage -eq 0) {
            ...
            "<keyUsageProperty>$([int]$temp.templatePSPKI.Settings.CNGKeyUsage)</keyUsageProperty>"
        }
        ...
        $PKS = if ([string]::IsNullOrEmpty($temp.templatePSPKI.Settings.Cryptography.PrivateKeySecuritySDDL)) {
            ...
            "<permissions>$($temp.templatePSPKI.Settings.PrivateKeySecuritySDDL)</permissions>"
        }
        ...
        $CSPs = if ($temp.templatePSPKI.Settings.Cryptography.CSPList.Count -eq 0) {
            ...
            $temp.templatePSPKI.Settings.Cryptography.CSPList | ForEach-Object {
        ...
        $RAR = if ($temp.templatePSPKI.Settings.RegistrationAuthority.SignatureCount -eq 0) {
            ...
            $str = @"
<rARequirements>
<rASignatures>$($temp.templatePSPKI.Settings.RegistrationAuthority.SignatureCount)</rASignatures>
"@
...
"@
        ...
        # key archival
        $KAS = if (!$temp.templatePSPKI.Settings.KeyArchivalSettings.KeyArchival) {
...
<keyArchivalAttributes>
    ...
    <symmetricAlgorithmKeyLength>$($temp.templatePSPKI.Settings.KeyArchivalSettings.KeyLength)</symmetricAlgorithmKeyLength>
</keyArchivalAttributes>
"@
        }
...
<attributes>
    ...
    $superseded
    <privateKeyFlags>$([int]$temp.templateADO.'msPKI-Private-Key-Flag'.FlagValue)</privateKeyFlags>
